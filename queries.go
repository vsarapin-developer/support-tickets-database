...
func GetTicketsCount(dbConfig DBConfig, progress *pb.ProgressBar) ([]string, map[string][4]uint) {
	...
	rows, err := db.Query(`
        WITH RECURSIVE date_series AS (
            SELECT '2023-11-01' AS created_date
            UNION ALL
            SELECT created_date + INTERVAL 1 DAY
            FROM date_series
            WHERE created_date <= '2023-11-30'
        ), hour_series AS (
	...
	rows, err := db.Query(`
		...
		LEFT JOIN
			inboxes ON DATE_FORMAT(inboxes.created_at, '%Y-%m-%d') = date_series.created_date
					AND inboxes.created_at >= '2023-11-01' AND inboxes.created_at <= '2023-11-30'
	...
	rows, err := db.Query(`
		...
		LEFT JOIN
			tickets_messages tm ON DATE_FORMAT(tm.created_at, '%Y-%m-%d') = date_series.created_date
							AND tm.author_id != 1
							AND tm.created_at >= '2023-11-01'
							AND tm.created_at <= '2023-11-30'
	...
	rows, err := db.Query(`
		...
		LEFT JOIN
			agent_kyc_becomes akb ON DATE_FORMAT(akb.created_at, '%Y-%m-%d') = date_series.created_date
						...
						AND akb.created_at >= '2023-11-01'
						AND akb.created_at <= '2023-11-30'
